...
const { ensureDir, rmrf, log, formatBytes } = require('./utils');
...
async function install(name, version, customPath, flags) {
  ...
  if (!pkg) {
    ...
    try { err.suggest = sources.suggest(name, 5).map(function (s) { return s.name; }); }
    catch (_) { err.suggest = []; }
    throw err;
  }
  ...
  const isInstaller = target.assetType === 'installer' || runner.isInstallerType(target.fileName);
  ...
  if (customPath && !isInstaller) log.info('安装路径: ' + path.resolve(customPath));
  ...
  if (isInstaller) {
    if (flags['no-run']) {
      ...
      } finally { tasks.unregister(instTask); }
      if (!flags.k) {
        try { fs.unlinkSync(dlDest); } catch (_) {}
  ...
  if (customPath) {
    ...
    finalDir = (base === pkg.name.toLowerCase()) ? resolved : path.join(resolved, pkg.name);
  } else {
  ...
  if (runner.isArchiveType(target.fileName)) {
    ...
    } catch (err) { log.error('解压失败: ' + err.message); }
  } else {
...
async function updatePackage(name, flags) {
  ...
  if (!installed.length) { log.info(i18n.t('noInstalledPkgs')); return 0; }

  const targets = name ? installed.filter(function (p) { return p.name === name; }) : installed;
  if (name && !targets.length) throw new Error(i18n.t('notInstalled') + ': ' + name);

  let count = 0;
  for (const inst of targets) {
    const pkg = sources.find(inst.name);
    if (!pkg || !pkg.latest) { log.warn(i18n.t('pkgNotFound') + ': ' + inst.name); continue; }
...
module.exports = {
  install, uninstall, updatePackage, addPackage, registerDisk, unregister
};
